...
function showMovieSendDoubleTrigger_cla(moviename, win, dio, first_trigger_value, second_trigger_frame, second_trigger_value,test )
...
%     elseif (frame_counter == second_trigger_frame)
        if~test
            WaitSecs(0.02);putvalue(dio, second_trigger_value);  
            WaitSecs(0.02);putvalue(dio,0);
        else
            disp('@@@@@@@@@ 2nd trigger @@@@@@@@@@@@@@');
        end
