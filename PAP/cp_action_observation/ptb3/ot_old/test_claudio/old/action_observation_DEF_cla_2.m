...
% 3 -> 4: AO + congruent sound (AOCS)
% 5 -> 8: AO + incongruent sound (AOIS)
% 6 -> 16: question
...
AOCS_sound_trigger_value=32;
AOIS_sound_trigger_value=64;
...
sound_onsets_frame={'45','45','45','45','45','45'};    % carta, bottiglia, tappo, pompetta, dadi, giraffa
ordered_sound_onsets_frame=repmat(sound_onsets_frame, 2, N_video_repetition);
sound_onsets_frame_list=cell(2,N_stimuli_x_cond);    
...
for row=3:4
    ...
    dd=row-2;
    input_row=ordered_sound_onsets_frame(dd,:);    
    sound_onsets_frame_list(dd,:)=input_row(permutation);
...
while 0
    ...
    [nx ny textbounds] = DrawFormattedText(window,'scenario naturale','center', 'center', white, 55 ,[],[],1.5);
    Screen('Flip', window);
    WaitSecs(2);
    rnd=ceil(rand()*N_stimuli_x_cond);
    file_video=fullfile(input_video_folder, [video_files_list{1,rnd} video_file_extension]);
    showMovieSendStartTrigger_cla_2(file_video, window, dio, control_trigger_value,test,trigger_duration);
%     if ~ test
...
%     end
    ...
    showMovieSendStartTrigger_cla_2(file_video, window, dio, AO_trigger_value,test,trigger_duration);
...
%     end
    ...
    showMovieSendDoubleTrigger_cla_2(file_video, window, dio, AOCS_trigger_value, sound_frame, AOCS_sound_trigger_value,test,trigger_duration);    % also send the frame at which send the second trigger
...
%     
    ...
    showMovieSendDoubleTrigger_cla_2(file_video, window, dio, AOCS_trigger_value, sound_frame, AOIS_sound_trigger_value,test,trigger_duration);    % also send the frame at which send the second trigger
...
% 8 -> 64: audio start in AOIS
        ...
        case 3
            ...
            sound_frame=sound_onsets_frame_list{1,aocs_trial};
            showMovieSendDoubleTrigger_cla_2(file_video, window, dio, AOCS_trigger_value, sound_frame, AOCS_sound_trigger_value,test,trigger_duration);    % also send the frame at which send the second trigger
        ...
        case 4
            ...
            sound_frame=sound_onsets_frame_list{2,aois_trial};
            showMovieSendDoubleTrigger_cla_2(file_video, window, dio, AOIS_trigger_value, sound_frame, AOIS_sound_trigger_value,test,trigger_duration);    % also send the frame at which send the second trigger
