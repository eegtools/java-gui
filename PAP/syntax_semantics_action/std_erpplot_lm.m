...
function [STUDY, erpdata, alltimes, pgroup, pcond, pinter] = std_erpplot_lm(STUDY, ALLEEG, varargin)
...
if ~isempty(opt.channels)
    ...
    if ~isempty(params.topotime) && all(~isnan(params.topotime))
        ...
        std_plotcurve_lm(alltimes, erpdata, 'groupstats', pgroup, 'legend', alllegends, 'condstats', pcond, 'interstats', pinter, ...
            'chanlocs', locs, 'titles', alltitles, 'plotsubjects', opt.plotsubjects, 'plotstderr', 'on', ...
    ...
    for index = 1:length(opt.clusters)
        ...
        std_plotcurve_lm(alltimes, erpdata, 'condnames', allconditions, 'legend', alllegends, 'groupnames', allgroups, 'plotstderr', 'on', ...
