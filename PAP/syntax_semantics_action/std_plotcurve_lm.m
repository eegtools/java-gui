...
function std_plotcurve_lm(allx, data, varargin) 
...
opt = finputcheck( varargin, { 'ylim'          'real'   []              [];
                               ...
                               'plotstderr'    'string' { 'on','off','diff','nocurve' }   'on';
...
for c = 1:ncplot
    for g = 1:ngplot
        ...
        if ~isempty(data{c,g})
            ...
            plotopt = { plotopt{:} 'ylim' [-4 4] 'xlabel' xlab 'ylabel' ylab };
...
%             elseif iscell(tmpdata)
                display('pippo')
                tmpdata
                plotcurve( allx, tmpdata{1}, 'ylim' ,[-4 4],'colors', tmpcol, 'maskarray', tmpdata{2}, plotopt{3:end}, 'title', opt.titles{c,g} );
...
%                 end;
                if ~strcmpi(opt.plotstderr, 'off') 
                    if ~dimreduced_sizediffers
                       ...
                       tmpdata_2=tmpdata{1}
                        for tmpi = begind:size(tmpdata_2,1)
                            display(tmpi);
                            tmpstd;
                             
                            hold on; chandle = fillcurves( allx, tmpdata_2(tmpi,:)-tmpstd(tmpi,:), tmpdata_2(tmpi,:)+tmpstd(tmpi,:), tmpcol{tmpi}); hold on;
