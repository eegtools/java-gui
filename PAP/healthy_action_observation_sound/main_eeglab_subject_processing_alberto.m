...
save_figure=0;
subjects_to_swap={'AC_10_cristina'};
...
subjects_list_emg_exec={'AC_01_alice','AC_02_chiara','AC_04_dan','AC_10_cristina','AC_11_claudio','AC_12_fabio','AC_13_emmanuel','AC_14_andrea','AC_15_alessandra'}; ... for debugging
subjects_list_emg_exec={'AC_04_dan'};
...
do_singlesubjects_betaband_comparison_execution=0;
%=====================================================================================================================================================================
%=====================================================================================================================================================================
%=====================================================================================================================================================================
%=====================================================================================================================================================================
...
if do_import_emg_exec
    for subj=1:length(subjects_list_emg_exec)
        eeglab_subject_import_event(project_settings, fullfile(bdf_path, [subjects_list_emg_exec{subj} '_emg' '.bdf']), emg_epochs_path, acquisition_system, nch_eeg, [],[],[],polygraphic_channels);
        file_name=fullfile(emg_epochs_path, [subjects_list_emg_exec{subj} '_emg' '_raw.set']);
        ...eeglab_subject_transform_poly(project_settings, file_name, epochs_path, nch_eeg, polygraphic_channels)
    end
end
...
if do_ica_emg_exec
    % do preprocessing up to epochs: avgref, epochs, rmbase: create one trails dataset for each condition
    for subj=1:length(subjects_list_emg_exec)
        file_name=fullfile(emg_epochs_path, [subjects_list_emg_exec{subj} '_emg' pre_epoching_input_file_name '.set']);
        eeglab_subject_ica(project_settings, file_name, emg_epochs_path, nch_eeg, 'cudaica');
    end
end
...
if do_epochs_execution
    % do preprocessing up to epochs: avgref, epochs, rmbase: create one trails dataset for each condition
     for subj=1:length(subjects_list_emg_exec)
         file_name=fullfile(emg_epochs_path, [subjects_list{subj} '_emg' pre_epoching_input_file_name '.set']);
         eeglab_subject_epoching_baseline_remove(project_settings, file_name, emg_epochs_path,'mrk_code', mrkcode_cond_execution,'mrk_name', name_cond_execution,'bc_st', bc_st_execution, 'bc_end', bc_end_execution);
     end
%     
     for subj=1:length(subjects_to_swap)
         input_file_name=fullfile(epochs_path, [subjects_to_swap{subj} '_emg' pre_epoching_input_file_name '.set']);
         output_file_name=fullfile(epochs_path, [subjects_to_swap{subj} '_sw' '_emg' pre_epoching_input_file_name '.set']);
         eeglab_subject_swap_electrodes(project_settings, input_file_name, output_file_name);
         eeglab_subject_epoching_baseline_remove(project_settings, output_file_name, emg_epochs_path);
     end
end
if do_singlesubjects_betaband_comparison_execution
     for subj=1:length(subjects_list_emg_exec)
        %electrode2inspect = electrode_of_interest{subj};
        
        cond1=fullfile(epochs_path, [subjects_list_emg_exec{subj} pre_epoching_input_file_name '_DFDR' '.set']);
        cond2=fullfile(epochs_path, [subjects_list_emg_exec{subj} pre_epoching_input_file_name '_NDFDR' '.set']);
        eeglab_subject_tf_plot_onecondition_vs_baseline(project_settings, cond1, electrode2inspect, stat_threshold, baseline_analysis_execution_interval_point, save_figure, results_path);
        eeglab_subject_tf_plot_onecondition_vs_baseline(project_settings, cond2, electrode2inspect, stat_threshold, baseline_analysis_execution_interval_point, save_figure, results_path);
