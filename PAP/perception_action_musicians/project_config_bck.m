...
% p-values
study_ls=0.1;%0.001;%0.0071;%0.00156;%0.00156;%0.005;%0.005;%NaN%
...
display_only_significant_curve='on'; %on

display_only_significant_topo='on'; %on
display_only_significant_topo_mode='surface'; %'electrodes';

display_only_significant_tf='on'; %on
display_only_significant_tf_mode='thresholded'; % 'binary';
...
ersp_mode='Pfu';% 'Pfu';  %%%%%%%%%% dB decibel, Pfu, (A-R)/R * 100 = (A/R-1) * 100 = (10^.(ERSP/10)-1)*100 variazione percentuale definita da pfursheller




masked_times_max_erp_curve=[];
filter_erp_curve=[];
% display (curve) plots with different conditions/groups on the same plots 
display_compact_plots_erp_curve='on';
% display parameters for compact plots
compact_display_h0_erp_curve='on';
compact_display_v0_erp_curve='on';
compact_display_sem_erp_curve='off';
compact_display_stats_erp_curve='on';
compact_display_xlim_erp_curve=[];
compact_display_ylim_erp_curve=[];

masked_times_max_ersp_curve_fb=[];
% display (curve) plots with different conditions/groups on the same plots 
...
correction='none';%'none';'fdr';%'holms';
...
for nband=1:length(frequency_bands_names)
 ...
 group_time_windows_list={{[100 600];[500 1000];[700 1100]};
                          {[100 600];[500 1000];[700 1100]};
                          {[100 600];[500 1000];[700 1100]};
     }; 
 
 group_time_windows_names={{'[100 600]';'[500 1000]';'[700 1100]'};...
                          {'[100 600]';'[500 1000]';'[700 1100]'};...
                          {'[100 600]';'[500 1000]';'[700 1100]'} ...
 }; 
 subject_time_windows_list={{[-100 100];[-100 100];[-100 100]};
                          {[-100 100];[-100 100];[-100 100]};
                          {[-100 100];[-100 100];[-100 100]};
     };
 
 which_method_find_extrema='single_subject';%'single_subject';%'group';'group_avg';
 
 % per gli erp bisogna definire il tipo di estremo per ogni design, roi e
 % finestra temporale
 which_extrema_erp_curve={...
                            {... design
                                {'max';'min';'min'};... roi
                                {'max';'min';'min'};...
                                {'max';'min';'min'}; ...
...
% per gli erp bisogna definire il tipo di estremo per ogni design, roi e
% finestra temporale (come per gli erp), ma anche per ogni banda
 which_extrema_ersp_curve_fb={ 
                                 {... design
                                    {... roi
                                        {'max';'min';'min'};... frequency band
                                        {'max';'min';'min'};...
                                        {'max';'min';'min'}; ...
                                        {'max';'min';'min'}...
                                    };... roi
...
sel_extrema='first_occurrence';%'avg_occurrences'
...
list_select_subjects={'2 carol active-Deci','3 andre active-Deci'};


set_caxis_ersp_tf=[];
set_caxis_erp_topo_tw=[];
set_caxis_ersp_topo_tw_fb=[];
timerange_ersp_tf=[min(timeout_analysis_interval),max(timeout_analysis_interval)];

freqrange_ersp_tf=[min(freqout_analysis_interval),max(freqout_analysis_interval)];

decimation_factor_times_ersp_tf=10;
decimation_factor_freqs_ersp_tf=10;


ersp_tf_resolution_mode='tw_fb';   %'continuous'; 'decimate_times';'decimate_freqs';'decimate_times_freqs';'tw_fb';
