...
x_offset = 0.5;
y_offset = 0.33;
incr_y=-10; %a volte, i point light passano proprio davanti alla croce di fissazione. per sistemare ciò, 
            %li si trasla verticalmente di incr_y, in modo tale che non
            %impiccino 
...
avi_file= 'scramble_random_speed.avi';%'LR_ankle_dritto_3sec.avi';
%---------------------------------------------
...
if strcmp(stimuli,'bounce')
    ...
    coord_2(2,:) = (H*H_ratio_1*(abs(sin((coord_1(1,:))*2*pi*freq_bounce_1+phase_bounce_1))));
    %(H*H_ratio_2*(abs(sin((coord_1(2,:))*2*pi*freq_bounce_2+phase_bounce_2))));
...
%traslo i point light verticalmente di incr_y pixels

pix_traj(2,:,:)=pix_traj(2,:,:)+incr_y;
...
if rallenty
    ...
    for i=1:13
        ...
        while counter1<268
            counter1=counter1+1;
            addendum=ceil(abs(267*rand(1)));
            Indexes(i,counter1)=addendum;
        end
        Indexes(i,:)=sort(Indexes(i,:));
        %    if size(Indexes(1,:),2)>268
        %        Indexes(1,:)=Indexes(i,1:268);
        %    end
    end
    for k=1:13 
    pix_traj(:,k,:)=pix_traj(:,k,Indexes(k,:));
    end
    % Indexes=1:268;
    %    for i=0:267
...
for iter = 1:N_iter
    if debug == 0
        ...
        % secondi del video; 
        if normal
        for i=1: round(monitor_freq*stimulus_time)%length(pix_traj) %
%             for j=1:2
...
%             Screen('FillOval', window, white, round([squeeze(W-pix_traj(1,:,i)) - dot_size ; squeeze(pix_traj(2,:,i)) - dot_size ; squeeze(W-pix_traj(1,:,i)) + dot_size ; squeeze(pix_traj(2,:,i)) + dot_size]));
...
%                 ANIM = getframe(rec_fig,[Lx Ly W-2*Lx H-2*Ly]);
            ...
            for i=1:600%length(pix_traj) %
%             for j=1:2
            Screen('DrawLines', window, cross_center, 3, red);
            % Commentare questa riga se si vuole solo la croce
            Screen('FillOval', window, white, round([squeeze(pix_traj(:,:,starting_fr)) - dot_size ; squeeze(pix_traj(:,:,starting_fr)) + dot_size]));  %creo il pallino blu
            Screen('FillOval', window, white, round([squeeze(W-pix_traj(1,:,starting_fr)) - dot_size ; squeeze(pix_traj(2,:,starting_fr)) - dot_size ; squeeze(W-pix_traj(1,:,starting_fr)) + dot_size ; squeeze(pix_traj(2,:,starting_fr)) + dot_size]));
