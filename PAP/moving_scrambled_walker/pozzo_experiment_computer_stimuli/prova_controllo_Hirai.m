...
isi_time=1.5; %inter stimulus time in seconds
...
if size(Indexes,2)>268
 ...
 for iter=1:10 %iter = Presentation
        ...
        phase_fr=1+floor(rand(1)*10)*7; %fattore vero che moltiplicherà sette: floor(rand(1)*10) [*7;]
        mean_init_x=mean(ankles(1,:,phase_fr),2);
        translation_x=[round(W/4-mean_init_x) ; 0];
        for i=phase_fr:phase_fr+stimulus_frames %initial_frame:initial_frame+stimulus_frames %1+shift_gait*walkershift(walkernb):stimulus_frames+shift_gait*walkershift(walkernb) 
                        Screen('DrawLines', window, cross_center, 3, red);
%                         Screen('FillOval', window, green, round([squeeze(walker(:,:,phase_fr))+repmat(translation_x,1,13) - dot_size ; squeeze(walker(:,:,phase_fr)) + repmat(translation_x,1,13) + dot_size]));
%                       
...
%                         Screen('DrawLines', window, cross_center, 3, red);
% %                         Screen('FillOval', window, white, round([squeeze(scramble(:,:,i)) - dot_size ; squeeze(scramble(:,:,i)) + dot_size]));
%                         Screen('FillOval', window, white, round([squeeze(W-scramble(1,:,i)) - dot_size ; squeeze(scramble(2,:,i)) - dot_size ; squeeze(W-scramble(1,:,i)) + dot_size ; squeeze(scramble(2,:,i)) + dot_size]));
% %                         Screen('FillOval', window, white, round([squeeze(scramble(:,:,Indexes(i))) - dot_size ; squeeze(scramble(:,:,Indexes(i))) + dot_size]));
% %                         Screen('FillOval', window, red, round([squeeze(inv_scramble2(:,test_markers(1),randomframes(1,i))) - dot_size ; squeeze(inv_scramble2(:,test_markers(1),randomframes(1,i))) + dot_size]));
% %                         
% %                         Screen('FillOval', window, red, round([squeeze(inv_scramble2(:,test_markers(2),randomframes(2,i))) - dot_size ; squeeze(inv_scramble2(:,test_markers(2),randomframes(2,i))) + dot_size]));
...
%                     for i=initial_frame:initial_frame+stimulus_frames %1+shift_gait*scrambleshift(scramblenb):stimulus_frames+shift_gait*scrambleshift(scramblenb) 
%                         Screen('DrawLines', window, cross_center, 3, red);
%                         Screen('FillOval', window, white, round([squeeze(W-ankles(1,:,i)) - dot_size ; squeeze(ankles(2,:,i)) - dot_size ; squeeze(W-ankles(1,:,i)) + dot_size ; squeeze(ankles(2,:,i)) + dot_size]));
%                         Screen('Flip', window); 
%                         if flag_record_avi
%                             %ANIM = getframe(rec_fig,[Lx Ly W-2*Lx H-2*Ly]);
%                             ANIM = getframe(rec_fig);
