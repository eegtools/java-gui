function [ersp times freqs pcond, pgroup, pinter] = eeglab_study_roi_tf_groupfreqs(STUDY, ALLEEG, channels_list, levels_f1, levels_f2,num_grouped_freqs,num_permutations,masked_times_max,paired,stat_method)

% function [ersp times freqs pcond, pgroup, pinter] = eeglab_study_roi_tf_groupfreqs(STUDY, ALLEEG, channels_list, levels_f1, levels_f2,num_grouped_freqs,correction,study_ls,num_permutations)
...
% masked_times_max is a time threshold (in ms): only times > masked_times_max will be considered for statistics on time-frequency representation.
    ...
    for nf1=1:length(levels_f1)
        for nf2=1:length(levels_f2)
            ...
            M=M(1:fixed_freqs,:,:);
            tmp_freqs_mat = reshape(M, [num_grouped_freqs final_freqs_mat  total_times total_subjects]);
            tmp_freqs_mat = squeeze(mean(tmp_freqs_mat));
            ersp{nf1,nf2} = reshape(tmp_freqs_mat, [final_freqs_mat total_times total_subjects]);
        end
    end
