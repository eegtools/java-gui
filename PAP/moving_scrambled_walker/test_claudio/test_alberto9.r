...
sig_threshold_patter=0.05
#folder of the project files
project_folder="D:/Download/plot-alberto/"
#define the name of the excel file with the data
xls_file=paste(project_folder,"moving_scrambled_walker_design1.xls",sep="")
#define the name of the file where results will be saved
res_file=paste(project_folder,"moving_scrambled_walker_design1_results.txt",sep="")
#define the name of the file where significant ersp are saved
sig_ersp_file=paste(project_folder,"sig_ersp_file.txt",sep="")
...
#
roi_list=list(
  c("F6","F8", "AF4","F8"),  #right IFG
  c("F5","F7", "AF3","F7"),  #right IFG
...
for(roi in roi_names){
  ...
  if (length(roi_list[[roi]]) > 1){
    vpow=rowMeans(raw_data[,roi_list[[roi]]],na.rm=T)
  }
  else{
    vpow=raw_data[,roi_list[[roi]]]
  }
...
###################################################################################################################################  
  ...
  for(roi_name in roi_names){
      ...
      ########################## PLOT PATTERNS
      file_plot=paste(project_folder,band_name,"_", roi_name,"_pattern",".tif",sep="")   
      ...
      for (cond_num in 1:length(cond_names)){
          ...
          sum_glht_baseline=summary(glht_baseline,test=adjusted(adjust_p_method))
          ...
          sig_var=sum_glht_baseline$test$pvalues<sig_threshold_patter
...
###################################################################################################################################  
  ...
  for(window_num in 1:length(window_names)){
    ...
    if(vp["roi"]<0.05){ 
      ...
      eval(parse(text=str))
      glht_aov_roi=summary(glht(aov_roi,linfct=mcp(roi="Tukey")),test=adjusted(adjust_p_method))
      str=paste(band_name,"_", window_name,"_" ,"glht_aov_roi","= glht_aov_roi",sep="")
      eval(parse(text=str))
      
      ttest_aov_roi=pairwise.t.test(ersp,roi, p.adjust.method = adjust_p_method,paired=T)
    ...
    eval(parse(text=str))
    glht_aov_motion=summary(glht(aov_motion,linfct=mcp(motion="Tukey")),test=adjusted(adjust_p_method))
    str=paste(band_name,"_", window_name,"_" ,"glht_aov_motion","= glht_aov_motion",sep="")
    eval(parse(text=str))
    ttest_aov_motion=pairwise.t.test(ersp,motion, p.adjust.method = adjust_p_method,paired=T)
    ...
    eval(parse(text=str))
    glht_aov_shape=summary(glht(aov_shape,linfct=mcp(shape="Tukey")),test=adjusted(adjust_p_method))
    str=paste(band_name,"_", window_name,"_" ,"glht_aov_shape","= glht_aov_shape",sep="")
    eval(parse(text=str))
    ttest_aov_shape=pairwise.t.test(ersp,shape, p.adjust.method = adjust_p_method,paired=T)
    ...
    if(vp["roi:motion"]<0.05){
      ...
      p=p.adjust(by(dmotion_ersp_motion,list(roi_motion),FUN=function(x) t.test(x)$p.value),method = adjust_p_method)      
...
#      
#      antpos_motion[grep("IFG",antpos_motion)] = "IFG"
#      antpos_motion[grep("IPL",antpos_motion)] = "IPL"
#      antpos_motion[grep("pSTS",antpos_motion)] = "pSTS"
#      antpos_motion[grep("bamt",antpos_motion)] = "bamt"
...
#      contr= c("IFG = 0","IPL = 0","pSTS = 0","bamt = 0")
#      glht_aov_antpos_motion=summary(glht(aov_antpos_motion,linfct=mcp(antpos_motion=contr)),test=adjusted(adjust_p_method))
...
#      p=p.adjust(by(dmotion_ersp_motion,list(antpos_motion),FUN=function(x) t.test(x)$p.value),method = adjust_p_method)      
...
#      #is the difference between rois depending from condition?
     ...
     for (name_motion in names_motion){
       ...
       eval(parse(text=str))    
       glht_aov_roi_sel_motion=summary(glht(aov_roi_sel_motion,linfct=mcp(roi_sel_motion="Tukey")),test=adjusted(adjust_p_method))
       str=paste(band_name,"_", window_name,"_" ,name_motion,"_glht_aov_roi_sel_motion","= glht_aov_roi_sel_motion",sep="")
       eval(parse(text=str))
       ttest_aov_roi_sel_motion=pairwise.t.test(ersp_sel_motion,roi_sel_motion, p.adjust.method = adjust_p_method,paired=T)
...
#        roi_dhemi[grep("IFG",roi_dhemi)] = "IFG"
#        roi_dhemi[grep("IPL",roi_dhemi)] = "IPL"
#        roi_dhemi[grep("pSTS",roi_dhemi)] = "pSTS"
#        roi_dhemi[grep("bamt",roi_dhemi)] = "bamt"
#        roi_dhemi=factor(roi_dhemi)
#        aov_roi_dhemi=aov(dhemi_ersp ~ roi_dhemi)
#        glht_aov_roi_dhemi=summary(glht(aov_roi_dhemi,linfct=mcp(roi_dhemi ="Tukey")),test=adjusted(adjust_p_method))
...
#                
        ...
        #ttest_aov_roi_dhemi=pairwise.t.test(dhemi_ersp,roi_dhemi, p.adjust.method = adjust_p_method,paired=T)
...
#        p=p.adjust(by(dhemi_ersp,list(roi_dhemi),FUN=function(x) t.test(x)$p.value),method = adjust_p_method)      
...
#     }
    ...
    file_plot=paste(project_folder,band_name,"_","motion_" ,window_name,"_barplot",".tif",sep="")   
    ...
    file_plot=paste(project_folder,band_name,"_","shape_" ,window_name,"_barplot",".tif",sep="")   
    tiff(file_plot,bg='transparent',width=550,height=480)    
    ...
    file_plot=paste(project_folder,band_name,"_","cond_" ,window_name,"_barplot",".tif",sep="")   
    tiff(file_plot,bg='transparent',width=550,height=480)    
