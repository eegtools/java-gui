% std_plotcurve_ersp() - plot average ESPS in a band as an ERP for a STUDY component 
%                   or channel cluster 
% Usage:
...
function std_plotcurve_ersp(allx, data, plot_dir, roi_name, frequency_band_name, name_f1, name_f2, levels_f1,levels_f2, varargin) 
...
% labels
% ------
% if strcmpi(opt.unitx, 'ms'), xlab = 'Time (ms)';      ylab = 'Power (10*log_{10}(\muV^{2}))';
% else                         xlab = 'Frequency (Hz)'; ylab = 'Power (10*log_{10}(\muV^{2}))'; 
if strcmpi(opt.unitx, 'ms'), xlab = 'Time (ms)';      ylab = 'dB';
...
tlf2=length(levels_f2);  
 ...
 if tlf1 > 1 && tlf2 <= 1
    suptitle([frequency_band_name ' ERSP in ' roi_name,': ', name_f1 ])
end

if tlf1 <=1 && tlf2 > 1
    suptitle([frequency_band_name ' ERSP in ' roi_name,': ', name_f2 ])    
end
if tlf1 > 1 && tlf2 > 1 
    suptitle([frequency_band_name ' ERSP in ' roi_name,': ', name_f1 ' and ' name_f2 ])   
end 

file_fig=['ersp_curve_',char(roi_name),'_',char(frequency_band_name),'.fig'];
saveas(fig, fullfile(plot_dir,file_fig));
print(fig,fullfile(plot_dir,'ersp_curve.ps'),'-append','-dwinc')


 os = system_dependent('getos');
 if ~ strncmp(os,'Linux',2) 
    saveppt2(fullfile(plot_dir,'ersp_curve.ppt'),'f',fig);
end
 
ps2pdf('psfile',fullfile(plot_dir,'ersp_curve.ps'),'pdffile',fullfile(plot_dir,'ersp_curve.pdf'))
