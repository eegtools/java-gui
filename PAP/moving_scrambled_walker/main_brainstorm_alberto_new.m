...
local_projects_data_path='/data/projects';  ... '/media/data/EEG';
eegtools_svn_local_path='/data/behavior_lab_svn/behaviourPlatform/EEG_Tools'; ... '/media/data/EEG/EEG_Tools';
plugins_path='/media/geo_repository/groups/behaviour_lab/Projects/EEG_tools/matlab_toolbox';
%==================================================================================
%==================================================================================
...
project_structure_filename='project_structure.m';
...
project_structure_path=fullfile(local_projects_data_path, project_folder, project_structure_filename);
[path,name_noext,ext] = fileparts(project_structure_path);
...
if isempty(bst_get('Protocol', project.brainstorm.db_name)) 
    iProtocol = brainstorm_protocol_create(project.brainstorm.db_name, brainstorm_db_path, default_anatomy);
else
...
if do_sources
    ...
    for nsubj=1:numsubj
        ...
        for cond=1:length(name_cond)
            ...
            sources_results{nsubj,cond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'wmne', 'orient', 'free', 'tag', 'vol', 'headmodelfile', vol_bem_file_name);
            sources_results{nsubj,cond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'wmne', 'orient', 'fixed', 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
            sources_results{nsubj,cond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'wmne', 'loose_value', 0.2, 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
            sources_results{nsubj,cond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'sloreta', 'orient', 'free', 'tag', 'vol', 'headmodelfile', vol_bem_file_name);
            sources_results{nsubj,cond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'sloreta', 'orient', 'fixed', 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
            sources_results{nsubj,cond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'sloreta', 'loose_value', 0.2, 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
        end
        % main effects
        for mcond=1:length(name_maineffects)
            ...
            sources_results{nsubj,totcond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'wmne', 'orient', 'free', 'tag', 'vol', 'headmodelfile', vol_bem_file_name);
            sources_results{nsubj,totcond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'wmne', 'orient', 'fixed', 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
            sources_results{nsubj,totcond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'wmne', 'loose_value', 0.2, 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
            sources_results{nsubj,totcond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'sloreta', 'orient', 'free', 'tag', 'vol', 'headmodelfile', vol_bem_file_name);
            sources_results{nsubj,totcond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'sloreta', 'orient', 'fixed', 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
            sources_results{nsubj,totcond} = brainstorm_subject_sources(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, 'norm', 'sloreta', 'loose_value', 0.2, 'tag', 'surf', 'headmodelfile', surf_bem_file_name);
        end        
    end
end
%==================================================================================
...
if do_sources_export2spm8
    ...
    for nsubj=1:numsubj
        disp(['exporting sources of ' subjects_list{nsubj}]);
        % 4 conditions
        for cond=1:length(name_cond)
            source_file=fullfile(subjects_list{nsubj}, name_cond{cond}, [src_name '.mat']);
            ...cond_input_file=fullfile(name_cond{cond}, [src_name '.mat']);
            for tw=1:length(time_windows_list{1}(tw))
            
                 output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' time_windows_names_short{1}(tw)];
                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, time_windows_sec{tw},  output_tag); ... 'alessandra_finisguerra_centered_N200')

%                  output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'BL']; ... '_' time_windows_names_short{1}(tw)];
%                  interval=[-0.396 -0.012];
%                  brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                  output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'P100']; ... '_' time_windows_names_short{1}(tw)];
%                  interval=[0.100 0.12];
%                  brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);

%                 output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'N200']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.200, 0.228];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
%                 
%                 output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'P330']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.300 0.360];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                 output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'N400']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.388 0.448];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                 output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'P500']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.448 0.548];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                 output_tag=[subjects_list{nsubj} '_' name_cond{cond} '_' src_name '_' 'N600']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.548 0.800];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
                
                
                %brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, '/media/data/EEG/moving_scrambled_walker/spm_sources', [0.2, 0.228], 'alessandra_finisguerra_centered_N200')
                %brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, spmsources_path, time_windows_list{1}(tw), output_tag);
            end
        end
        % main effects
        for mcond=1:length(name_maineffects)
            totcond=mcond+length(name_cond);
            source_file=fullfile(subjects_list{nsubj}, name_maineffects{mcond}, [src_name '.mat']);
%             cond_input_file=fullfile(name_maineffects{mcond}, [src_name '.mat']);
            
%                  output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'BL']; ... '_' time_windows_names_short{1}(tw)];
%                  interval=[-0.396 -0.012];
%                  brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                  output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'P100']; ... '_' time_windows_names_short{1}(tw)];
%                  interval=[0.100 0.12];
%                  brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
%             
%                 output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'N200']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.200 0.228];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
%                 
%                 output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'P330']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.300 0.360];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                 output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'N400']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.388 0.448];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                 output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'P500']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.448 0.548];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);
% 
%                 output_tag=[subjects_list{nsubj} '_' name_maineffects{mcond} '_' src_name '_' 'N600']; ... '_' time_windows_names_short{1}(tw)];
%                 interval=[0.548 0.800];
%                 brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, source_file, spmsources_path, interval, output_tag);   
                
            ...for tw=1:length(time_windows_list{1})
                ...output_tag=[subjects_list{nsubj} '_' name_maineffects{cond} '_' src_name '_' time_windows_names_short{1}(tw)];
               ... brainstorm_subject_sources_export2spm_volume(project_settings, protocol_name, subjects_list{nsubj}, cond_input_file, spmsources_path, time_windows_list{1}(tw), output_tag);
            ...end            
         end        
    end    
