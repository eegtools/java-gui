...
% EXAMPLE
% = struct('visible', 'on', 'type', 'label', 'canbeempty', 'off', 'default', '', 'values', {}, 'onChange', @callback);
...
%                   'input_type'='string/num/all'

% - input_array     free array, type of array (numeric or cell) must be specified : 
%                   'output_type'='cell/numeric';   

% - combobox        value selected from a list
%                   'values'= {'a', 2, 'c',...}

% - select listbox  values selected from a list, default is a cell array
%                   'values'= {'a', 2, 'c',...}

% - input listbox   values added with a button 'add' to a list..so edit and remove button should be also present, default is a cell array, 'input_type'='string/num/all'

% - checkbox        values 'on' or 'off'

% - input datagrid (two dimensional cell array), fields is a cell array of
%                   'fields'
%                   string, cell array of input_type for each field, fixed fields 'on','off', if off show add/remove/rename buttons
% 
% DEFAULT:
% - free string
% - cell array
...
% input
projectgui.import.acquisition_system       = struct('visible', 'on', 'type', 'combobox', 'canbeempty', 'off', 'default', '', 'values', {'BIOSEMI', 'BRAINVISION', 'EDF'});                    % D1:   EEG hardware type: BIOSEMI | BRAINAMP
projectgui.import.original_data_extension  = struct('visible', 'on', 'type', 'label', 'canbeempty', 'off', 'default', '');                        % D2:   original data file extension BDF | vhdr
projectgui.import.original_data_folder     = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', '');            % D3:   original data file subfolder
projectgui.import.original_data_suffix     = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', '');                       % D4:   string after subject name in original EEG file name....often empty
projectgui.import.original_data_prefix     = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', '');                          % D5:   string before subject name in original EEG file name....often empty


% output
projectgui.import.output_folder            = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', projectgui.import.original_data_folder.default); % D6:   string appended to fullfile(projectgui.paths.projectgui,'epochs', ...) , determining where to write imported file, by default coincides with original_data_folder
projectgui.import.output_suffix            = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', '');                           % D7:   string appended to input file name after importing original file
projectgui.import.emg_output_postfix       = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', '');                  			% D8:   string appended to input file name to EMG file

projectgui.import.reference_channels       = struct('visible', 'on', 'type', 'combobox', 'canbeempty', 'on', 'default', [], 'values', {[], 'CAR', 'sel elec'});                      % D9:   list of electrodes to be used as reference: []: no referencing, {'CAR'}: CAR ref, {'el1', 'el2'}: used those electrodes

% D10:   list of electrodes to transform
projectgui.import.ch2transform              = struct('visible', 'on', 'type', 'input', 'canbeempty', 'on', 'default', [], 'values', [], 'fields', {'type','ch1','ch2','new_label'}, 'edit_row','on','edit_cell','on','edit_field','off');

% D11:  list of trigger marker to import. can be a cel array, or a string with these values: 'all', 'stimuli','responses'
projectgui.import.valid_marker             = struct('visible', 'on', 'type', 'input_array', 'canbeempty', 'off', 'values', [], 'default', {}, 'onChange', [], 'output_type', 'cell');  

%% ======================================================================================================
% E:    FINAL EEGDATA
% ======================================================================================================
projectgui.eegdata.nch                         = struct('visible', 'on', 'type', 'input', 'canbeempty', 'off', 'values', [], 'default', '');                           % E1:   final channels_number after electrode removal and polygraphic transformation
projectgui.eegdata.nch_eeg                     = struct('visible', 'on', 'type', 'input', 'canbeempty', 'off', 'values', [], 'default', '');                           % E2:   EEG channels_number
projectgui.eegdata.fs                          = struct('visible', 'on', 'type', 'input', 'canbeempty', 'off', 'values', [], 'default', '');                          % E3:   final sampling frequency in Hz, if original is higher, then downsample it during pre-processing
projectgui.eegdata.eeglab_channels_file_name   = struct('visible', 'on', 'type', 'input', 'canbeempty', 'off', 'values', [], 'default', 'standard-10-5-cap385.elp');     % E4:   universal channels file name containing the position of 385 channels
projectgui.eegdata.eeglab_channels_file_path   = struct('visible', 'on', 'type', 'label', 'canbeempty', 'on', 'values', [], 'default', '');                           % E5:   later set by define_paths
    
projectgui.eegdata.eeg_channels_list           = struct('visible', 'off', 'type', 'label', 'canbeempty', 'on', 'values', [], 'default', '');  % E6:   list of EEG channels IDs

projectgui.eegdata.emg_channels_list           = struct('visible', 'on', 'type', 'label', 'canbeempty', 'on', 'values', [], 'default', 'selfvar');
projectgui.eegdata.emg_channels_list_labels    = struct('visible', 'on', 'type', 'label', 'canbeempty', 'on', 'values', [], 'default', 'selfvar');
...
projectgui.stats.eeglab.erp.method                 = struct('visible', 'on', 'type', 'combobox', 'default', {'bootstrap', 'parametric', 'permutation'});          % method applied in ERP statistical analysis
...
projectgui.stats.eeglab.ersp.method                = struct('visible', 'on', 'type', 'combobox', 'default', {'bootstrap', 'parametric', 'permutation'});          % method applied in ERP statistical analysis
