...
%% Init CDAQ library
protocol_param.cdaq_config_file='F:\WorkingDir\EEG_Tools\eeg_bar_dot\ptb3\DanLiAcq.cfg';
if ~cdaq_init(protocol_param.cdaq_config_file)
    return; 
end
address = hex2dec('378'); 
...
mkdir(protocol_param.datadir,subject_name);
protocol_param.subjectdir = fullfile(protocol_param.datadir, subject_name);
% Add Trial dir using date
mkdir(protocol_param.subjectdir,date);
protocol_param.trialdir = fullfile(protocol_param.subjectdir, date);
screen_param = create_screen_struct;
% Create default accel_data
output_accel_data_path=fullfile(protocol_param.trialdir, 'accell_data.mat');
save(output_accel_data_path, 'subject_name');
...
[DOT,BAR,CROSS] = create_stimuli_struct(screen_param,protocol_param,pointing_data_normalized);
...
save([protocol_param.trialdir '\all\stimuli.mat'], 'DOT', 'BAR', 'CROSS');
