...
configFile='F:\WorkingDir\EEG_Tools\point_dot_bar\ptb3\DanLiAcq.cfg';
...
%These functions don't work because printf doesn't appear on matlab consolle
%calllib('cdaq', 'cdaqPrintConfigSettings')
%disp('Sources:')
%calllib('cdaq','cdaqListSources')
...
while done==0 && ishandle(H)
    if calllib('cdaq','cdaqRead')==0 || calllib('cdaq','cdaqIsTaskDone',done)==0
